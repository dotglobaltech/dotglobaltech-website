<template>
  <section class="about__area p-relative z-index-1 pt-50 pb-50">
    <div class="container">
      <div class="row justify-content-center">
        <div class="col-xxl-8 col-xl-8 col-lg-10 col-md-10">
          <div class="section__title-wrapper-7 mb-60 text-center">
            <div class="section__title-wrapper-7 mb-40">
              <h3 class="section__title-7">
                Cloud Migration and Modernization,
                <span class="section__title-7-highlight">
                  Made Seamless
                  <svg width="240" height="22" viewBox="0 0 240 22" fill="none">
                    <path fill-rule="evenodd" clip-rule="evenodd"
                      d="M0.440833 21.1152C78.9497 6.16225 158.05 4.53412 235.949 13.8239C237.497 14.0088 239.796 12.4065 239.988 9.93474C240.181 7.4176 238.026 5.44088 236.474 5.2332C157.99 -5.31675 79.1936 0.359501 0.316568 19.7785C-0.184784 19.9023 -0.0511379 21.2092 0.440833 21.1152Z"
                      fill="currentColor" />
                  </svg>
                </span>
              </h3>
            </div>
          </div>
        </div>
      </div>
      <div class="row">
        <div class="col-xxl-6 col-lg-6">
          <div class="about__thumb-wrapper-14 p-relative" data-sal="slide-up" data-sal-delay="130"
            data-sal-duration="1000">
            <div class="about__shape">
              <img class="about__shape-8" src="~/assets/img/about/14/about-shape-1.png" alt="">
              <img class="about__shape-9" src="~/assets/img/about/14/about-shape-2.png" alt="">
            </div>
            <div class="about__thumb-14 m-img">
              <img class="about-img-1" src="~/assets/img/image-here.png" alt="">
            </div>
          </div>
        </div>
        <div class="col-xxl-6 col-lg-6">
          <div class="about__wrapper-7 pt-35">
            <div class="about__list about__list-counter">
              <div class="about__list-item d-flex align-items-start">
                <div class="about__list-icon">
                  <span></span>
                </div>
                <div class="about__list-content">
                  <h3 class="about__list-title">Fearless workload migration</h3>
                  <p>
                    Our experts handle the heavy lifting, meticulously transferring your data and applications to AWS
                    with minimal disruption.
                  </p>
                </div>
              </div>
              <div class="about__list-item d-flex align-items-start">
                <div class="about__list-icon">
                  <span></span>
                </div>
                <div class="about__list-content">
                  <h3 class="about__list-title">Infrastructure refresh</h3>
                  <p>
                    Ditch the outdated and embrace the cutting-edge. We modernize your systems with scalable, secure
                    cloud solutions built for the future through our well-designed <strong>AWS cloud managed
                      services.</strong>
                  </p>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>

    <div class="container pt-80">
      <div class="row justify-content-center">
        <div class="col-xxl-8 col-xl-8 col-lg-10 col-md-10">
          <div class="section__title-wrapper-7 mb-60 text-center">
            <div class="section__title-wrapper-7 mb-40">
              <h3 class="section__title-7">
                App Development and Deployment:
                <span class="section__title-7-highlight">
                  A Launchpad for Innovation
                  <svg width="240" height="22" viewBox="0 0 240 22" fill="none">
                    <path fill-rule="evenodd" clip-rule="evenodd"
                      d="M0.440833 21.1152C78.9497 6.16225 158.05 4.53412 235.949 13.8239C237.497 14.0088 239.796 12.4065 239.988 9.93474C240.181 7.4176 238.026 5.44088 236.474 5.2332C157.99 -5.31675 79.1936 0.359501 0.316568 19.7785C-0.184784 19.9023 -0.0511379 21.2092 0.440833 21.1152Z"
                      fill="currentColor" />
                  </svg>
                </span>
              </h3>
            </div>
          </div>
        </div>
      </div>
      <div class="row">
        <div class="col-xxl-6 col-lg-6">
          <div class="about__wrapper-7 pt-35">
            <div class="about__list about__list-counter">
              <div class="about__list-item d-flex align-items-start">
                <div class="about__list-icon">
                  <span></span>
                </div>
                <div class="about__list-content">
                  <h3 class="about__list-title">Scalable mobile and web apps</h3>
                  <p>
                    Craft secure, high-performance applications that adapt to your user base like magic, built on the
                    rock-solid foundation of AWS.
                  </p>
                </div>
              </div>
              <div class="about__list-item d-flex align-items-start">
                <div class="about__list-icon">
                  <span></span>
                </div>
                <div class="about__list-content">
                  <h3 class="about__list-title">Rapid deployment</h3>
                  <p>
                    Get your solutions to market faster than ever with streamlined deployment processes for a seamless
                    transition from concept to reality. We are the best <strong>managed service provider</strong>
                    catering to your business-specific cloud technology needs.
                  </p>
                </div>
              </div>
            </div>
          </div>
        </div>
        <div class="col-xxl-6 col-lg-6">
          <div class="about__thumb-wrapper-14 p-relative" data-sal="slide-up" data-sal-delay="130"
            data-sal-duration="1000">
            <div class="about__shape">
              <img class="about__shape-8" src="~/assets/img/about/14/about-shape-1.png" alt="">
              <img class="about__shape-9" src="~/assets/img/about/14/about-shape-2.png" alt="">
            </div>
            <div class="about__thumb-14 m-img">
              <img class="about-img-1" src="~/assets/img/image-here.png" alt="">
            </div>
          </div>
        </div>
      </div>
    </div>

    <div class="container pt-80">
      <div class="row justify-content-center">
        <div class="col-xxl-8 col-xl-8 col-lg-10 col-md-10">
          <div class="section__title-wrapper-7 mb-60 text-center">
            <div class="section__title-wrapper-7 mb-40">
              <h3 class="section__title-7">
                Cutting-Edge Cloud-Native Solutions: 
                <span class="section__title-7-highlight">
                  Pushing the Boundaries
                  <svg width="240" height="22" viewBox="0 0 240 22" fill="none">
                    <path fill-rule="evenodd" clip-rule="evenodd"
                      d="M0.440833 21.1152C78.9497 6.16225 158.05 4.53412 235.949 13.8239C237.497 14.0088 239.796 12.4065 239.988 9.93474C240.181 7.4176 238.026 5.44088 236.474 5.2332C157.99 -5.31675 79.1936 0.359501 0.316568 19.7785C-0.184784 19.9023 -0.0511379 21.2092 0.440833 21.1152Z"
                      fill="currentColor" />
                  </svg>
                </span>
              </h3>
            </div>
          </div>
        </div>
      </div>
      <div class="row">
        <div class="col-xxl-6 col-lg-6">
          <div class="about__thumb-wrapper-14 p-relative" data-sal="slide-up" data-sal-delay="130"
            data-sal-duration="1000">
            <div class="about__shape">
              <img class="about__shape-8" src="~/assets/img/about/14/about-shape-1.png" alt="">
              <img class="about__shape-9" src="~/assets/img/about/14/about-shape-2.png" alt="">
            </div>
            <div class="about__thumb-14 m-img">
              <img class="about-img-1" src="~/assets/img/image-here.png" alt="">
            </div>
          </div>
        </div>
        <div class="col-xxl-6 col-lg-6">
          <div class="about__wrapper-7 pt-35">
            <div class="about__list about__list-counter">
              <div class="about__list-item d-flex align-items-start">
                <div class="about__list-icon">
                  <span></span>
                </div>
                <div class="about__list-content">
                  <h3 class="about__list-title">Serverless revolution</h3>
                  <p>
                    Ditch the heavy server management and embrace the future of cloud computing with serverless architectures built for agility and cost efficiency. We execute every <strong>AWS implementation</strong> project by leveraging product expertise to ensure customer success.
                  </p>
                </div>
              </div>
              <div class="about__list-item d-flex align-items-start">
                <div class="about__list-icon">
                  <span></span>
                </div>
                <div class="about__list-content">
                  <h3 class="about__list-title">IoT and ML powerhouses</h3>
                  <p>
                    Connect the dots with groundbreaking IoT solutions and harness the power of machine learning to unlock deeper insights and drive intelligent decisions.
                  </p>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  </section>
</template>

<script>
export default {
  name: 'Why Choose Us'
};
</script>